...
// Returns -1 on error.
int openSerialPort(const char *serialPort, int baudRate, int *fd, struct termios *oldtio) {
...
int readByteSerialPort(unsigned char *byte, int fd) { return read(fd, byte, 1); }
...
int writeBytesSerialPort(const unsigned char *bytes, int nBytes, int fd) {
