...
#ifndef CUSTOM_BAUDRATE
#define DEFAULT_BAUDRATE 9600
#define BAUDRATE B9600
#else
#define DEFAULT_BAUDRATE CUSTOM_BAUDRATE
static speed_t get_baudrate_macro(int baudrate) {
  switch (baudrate) {
  case 1200:
    return B1200;
  case 1800:
...
struct Parameters par = {.cableOn = TRUE,
...
int openSerialPort(const char *serialPort, struct termios *oldtio,
...
#ifdef CUSTOM_BAUDRATE
  newtio->c_cflag = get_baudrate_macro(CUSTOM_BAUDRATE) | CS8 | CLOCAL | CREAD;
#else
...
int init_ring_buffers(void) {
  ...
  if (par.tx2rx == NULL || par.tx2rxValid == NULL || par.rx2tx == NULL ||
      par.rx2txValid == NULL) {
    return -1;
  }
...
struct timespec timespec_diff(const struct timespec *t2,
...
struct timespec timespec_sum(const struct timespec *t1,
...
void help() {
  printf(
      ...
      "--- ber <ber>    : add noise to data bits at a specified BER "
      "(default=0)\n"
      "--- baud <rate>  : set baud rate, between 1200 and 115200 "
      "(default=9600)\n"
      "                   note that 10 bits are sent per byte (8-N-1)\n"
      "--- prop <delay> : set the propagation delay in usec (0-1000000, "
      "default=0)\n"
      "                   will be approximated to an integer multiple of the "
      "byte\n"
      "                   delay (10 / baud_rate)\n"
      ...
      "\n"
      "IMPORTANT: Changing the baud rate or propagation delay while a "
      "transmission is\n"
...
int main(int argc, char *argv[]) {
  ...
  system("socat -dd PTY,link=" TXDEV
         ",mode=777,raw,echo=0 PTY,link=" TX_EMULATOR ",mode=777,raw,echo=0 &");
  sleep(1);
  printf("\n");

  system("socat -dd PTY,link=" RXDEV
         ",mode=777,raw,echo=0 PTY,link=" RX_EMULATOR ",mode=777,raw,echo=0 &");
  sleep(1);
  ...
  while (STOP == FALSE) {
    ...
    if (par.cableOn) {
      if (par.tx2rxValid[par.tx2rxIdx]) {
        ...
        if (par.byteER != 0.0 &&
            (double)rand() / (double)RAND_MAX < par.byteER) {
    ...
    if (fromStdin > 0) {
      ...
      if (strcmp(rxStdin, "off") == 0) {
        ...
        if (ber >= 0.0 && ber < 1.0) {
          ...
          if (ber > 0.01) {
            printf("   ACTUAL BER WILL BE LOWER THAN DEFINED FOR VALUES ABOVE "
                   "0.01\n");
          }
        } else {
        ...
        case 115200:
          ...
          break;
        default:
          printf("UNSUPPORTED BAUD RATE: must be one of 1200, 1800, 2400, "
